...
public class SearchMetadataHandler extends RequestHandler {
    private static Log log = LogFactory.getLog(SearchMetadataHandler.class);
	...
	public SearchMetadataHandler(GetNameInfoDataMessage requestMsg) throws I2B2Exception  {
		try {
		...
		*/
		...
		if (response != null && response.size()>0 && vocabType.isReducedResults()!=null && vocabType.isReducedResults()) {
			...
			while (it.hasNext())
			{
				...
				for (String k : seen) {
					if(key.startsWith(k) && !key.equals(k) ) {
						bAbort = true;
						break;
					}
				}
		...
		//responseMessageType = MessageFactory.doBuildErrorResponse(nameInfoMsg.getMessageHeaderType(), response);
		if (responseVdo == null)
			responseVdo = MessageFactory.convertToXMLString(responseMessageType);
		return responseVdo;
